...
- Make `Type.lub` return a true upper bound when exactly one side is refined; `A(HAS R) lub A` yields `A(HAS R)`, which `A` does not narrow (`docs/agents/TYPES.md` §15.2).
