...
- **Medium-high priority:** Rethink the action model end to end before making local ownership fixes:
  `UseAction` is generated by generic Pets code, but its declaration and meaning are player-bound
  Terraforming Mars behavior. Do not move it into `system.pets` merely to erase that tension. The
  resulting model must also explain workflow sequencing, completion scopes such as Head Start's two
  non-interleavable actions, and the derivation of `Owed`, `Payment`, and `CostPaid` from paid action
  instructions.
