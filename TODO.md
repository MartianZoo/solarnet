...
- **High priority:** Separate task semantics from presentation order. Retain a deterministic view,
  ...
  instruction match, and let client-selected `first` remain a valid unsophisticated policy over the
  executable tasks in that view. Add a seeded random policy that chooses an executable task rather
  than randomizing stored positions. Autoexecution policy belongs outside the engine as specified in
