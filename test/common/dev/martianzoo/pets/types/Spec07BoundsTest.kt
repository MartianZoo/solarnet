...
internal class Spec07BoundsTest {
  ...
  // 7-1 Greatest lower bound

  @Test
  internal fun `7-1 glb of comparable types is the narrower one`() {
  ...
  @Test
  internal fun `7-1 glb intersects the root classes and every dependency`() {
  ...
  @Test
  internal fun `7-1 glb is absent when the classes have no unique common subclass`() {
    (type("GreeneryTile") glb type("OceanTile")) shouldBe null
    (type("LandArea") glb type("WaterArea")) shouldBe null
  }

  @Test
  internal fun `7-1 glb is absent when one dependency pair has no common narrowing`() {
  ...
  @Test
  internal fun `7-1 when glb exists it narrows both operands`() {
    sample.forEach { a ->
      sample.forEach { b ->
  ...
  @Test
  internal fun `7-1 when glb exists it is the greatest such type in the sample`() {
    sample.forEach { a ->
      sample.forEach { b ->
        val bound = type(a) glb type(b)
        sample.forEach { c ->
  ...
  // 7-2 Least upper bound

  @Test
  internal fun `7-2 lub of comparable types is the wider one`() {
  ...
  @Test
  internal fun `7-2 lub joins the root classes and the dependencies they share`() {
  ...
  @Test
  internal fun `7-2 lub always exists, falling back to Component`() {
    (type("Tharsis_2_2") lub type("Player1")) shouldBe type("Component")
  }

  @Test
  internal fun `7-2 lub is an upper bound of both operands`() {
    sample.forEach { a ->
      sample.forEach { b ->
  ...
  @Test
  internal fun `7-3 both operations are idempotent`() {
    sample.forEach { a ->
  ...
  @Test
  internal fun `7-3 both operations are commutative on structural types`() {
    sample.forEach { a ->
      sample.forEach { b ->
  ...
  @Test
  internal fun `7-3 lub may pick a different tied candidate when the operands swap`() {
    val table =
        loadTypes(
            "ABSTRACT CLASS Tile",
    ...
    // Neither `Tile` nor `GlobalParameter` is more minimal than the other, and rule 2-9 breaks the
    // tie by the order each operand happens to list its supertypes.
    (left lub right) shouldBe table.resolve(te("Tile"))
    (right lub left) shouldBe table.resolve(te("GlobalParameter"))
  }

  @Test
  internal fun `7-3 glb writes a joined requirement in operand order`() {
    val table =
        loadTypes(
  ...
  @Test
  internal fun `7-4 bounds across universes are rejected rather than answered`() {
