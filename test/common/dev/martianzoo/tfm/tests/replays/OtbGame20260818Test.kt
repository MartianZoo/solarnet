...
internal class OtbGame20260818Test : AbstractFullGameTest() {
  ...
  internal fun otbGame20260818() {
    ...
    otbGame20260818Replay.restore().endState() shouldBe game.endState()
    game.export().restore().endState() shouldBe game.endState()
  }

  private fun String.restore(): World =
      ScriptSession()
          .also { session ->
  ...
  @Test
  internal fun replayRestoreRejectsCommandErrors() {
    shouldThrow<IllegalArgumentException> { "do tasks(Plant)".restore() }
  }

  @Test
  internal fun playCardRejectsConsequenceArguments() {
    val replay =
        otbGame20260818Replay.substringBefore("DO playCard(Biofuels)") +
            "DO playCard(Biofuels, PROD[Plant, Energy])"

    shouldThrow<IllegalArgumentException> { replay.restore() }.message.orEmpty() shouldContain
        "playCard accepts only cost arguments"
  }

  private fun World.endState(): Map<String, Any> =
      mapOf(
          "actors" to actors,
          "classes" to classTable.allClassNames,
          "tasks" to tasks.extract { it.toString() }.sorted(),
          "components" to
              reader.getComponents("Component").entries.associate { (type, count) ->
                type.expressionFull.toString() to count
              },
      )
}
