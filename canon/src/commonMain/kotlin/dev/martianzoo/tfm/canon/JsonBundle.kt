 ...
 * produce a warning. When Pets files are present, their declarations must include the bundle's own
 * [name]. When none are present, that declaration is generated automatically. Callers whose
...
public class JsonBundle(
  ...
  override val explicitClassDeclarations: Set<ClassDeclaration> by lazy {
    val petsFilenames = resourceFilenames.filter { it.endsWith(PETS_EXTENSION) }.sorted()
    if (petsFilenames.isEmpty()) {
      setOf(parseOneLinerClass("CLASS $bundleName : System { HAS =1 This }"))
    } else {
      petsFilenames
          .flatMap { parseClasses(read(it)) }
          .toSetStrict()
          .also { declarations ->
            require(declarations.any { it.className == bundleName }) {
              "$resourceDirectory must declare CLASS $bundleName"
            }
          }
    }
  }
  ...
  override val marsMapDefinitions: Set<MarsMapDefinition> by lazy {
    if (MAPS_FILENAME in resourceFilenames) {
      JsonReader.readMaps(read(MAPS_FILENAME), requireNotNull(legacyCode)).toSetStrict()
    } else {
      emptySet()
    }
  }
