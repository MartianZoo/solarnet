...
import dev.martianzoo.repl.welcome
...
internal class JlineRepl(private val session: ReplSession) : ReplTerminal {
  private val interactiveCommands: Map<String, ReplCommand> =
      listOf(HistoryCommand(this)).associateBy { it.name }
  private val completer: Completer = JlineReplCompleter(session, interactiveCommands.values.toList())
  ...
  private fun execute(input: String): List<String> {
    ...
    return when (commandName.lowercase()) {
      "help" -> interactiveHelp(arguments.trim().ifEmpty { null })
      else -> {
        val command = interactiveCommands[commandName.lowercase()]
        if (command == null) {
  ...
  private fun interactiveHelp(args: String?): List<String> =
      when (args?.lowercase()) {
        "exit" -> listOf("I mean it exits.")
        "rebuild" -> listOf("Exits, recompiles the code, and restarts. Your game is lost.")
        "history" -> interactiveCommands.getValue("history").help.trimIndent().split("\n")
