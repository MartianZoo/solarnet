...
internal class Effector @Inject constructor(readerP: Provider<SnReader>) {
  private val reader: SnReader by lazy(readerP::get)
    ...
    private data class Personal(
      ...
      val player: Player? =
          try {
            ...
            require(by == SpecialClassNames.OWNER || by == SpecialClassNames.ANYONE)
            null
          }
      ...
      override fun checkForHit(
        ...
        return if (by == SpecialClassNames.OWNER) {
          { Transforming.replaceOwnerWith(actor).transform(originalHit(it)) }
        } else {
          originalHit
        }
      }
