...
class BugsTest : CardTest() {
  ...
  fun `solo setup can incorrectly link the second greenery to the first city`() {
    val setup =
        canonicalPremise("TerraformingMars,TharsisMapOption,SoloMode,CorporateEraExpansion", 1)
  ...
  fun `Head Start incorrectly allows its two actions to interleave`() {
    newGame("TerraformingMars,TharsisMapOption,PreludeExpansion,TurmoilCardPack,PromoCardPack")
    p1.phase("Prelude")
