...
Funded awards are `Award` components. The standard action charges 8, 14, then 20 megacredits as one cost; invariants prevent an award from being funded twice and limit the game to three funded awards. Award definitions and scoring classes are omitted from solo games. During multiplayer `EndPhase`, each funded award records a persistent `AwardTally` for every player, then records `FirstPlace` and, in games with at least three players, `SecondPlace` components. Those placement components award 5 and 2 victory points themselves. Ties for first receive 5 points each and suppress second place; ties for second receive 2 points each.
