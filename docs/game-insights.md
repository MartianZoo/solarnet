...
* `GreeneryTile` owns its oxygen reaction, conditioned on the singleton `Photosynthesis` component. `CorporationPhase` creates `Photosynthesis`, and `FinalGreeneryPhase` removes it, so setup and final greeneries do not raise oxygen. `DefaultGreeneryTile` owns the standard placement preference: next to one of the player's tiles when possible, otherwise on any empty land area.
