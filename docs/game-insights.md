...
* Some gains/removes are "atomized" and some aren't. That is, any time you raise the temperature 2 steps, or gain 2 science tags, you have to address each one as its own independent event, or the right things won't happen. However, if (say) reducing plant production 2 steps, those have to be done as a single change, or else Mons Insurance would fire twice. Currently, Tags and GlobalParameters are the only things marked as Atomized. Atomization produces comma siblings rather than a `THEN` chain so that each unit has an independent linkage scope; in particular, two ocean tiles must be placeable on different areas. (OceanTile inherits that bit, although it would have required atomization anyway.)
...
* The game models greenery oxygen with a singleton `Photosynthesis` component rather than an intrinsic `GreeneryTile` effect. `CorporationPhase` creates `Photosynthesis`, and `FinalGreeneryPhase` removes it, so setup greeneries and final greeneries don't raise oxygen while normal in-game greeneries still do. This is especially relevant to solo games that use TR63 mode.
