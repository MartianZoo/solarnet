...
* The game models greenery oxygen with a singleton `Photosynthesis` component rather than an intrinsic `GreeneryTile` effect. `CorporationPhase` creates it and `FinalGreeneryPhase` removes it, so setup and final greeneries don't raise oxygen but normal in-game greeneries do.
