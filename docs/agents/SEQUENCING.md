...
| Does this concern `EACH`, continuations, idle settlement, or atomicity? | Read only the matching explicitly proposed/exploratory section |
...
- **Immediacy:** after A, work cannot be postponed behind a player choice. Automatic `::` expresses
  this.
- **Idle settlement:** B is completion work that should arise only after a Player's current queue
  drains. The owned `Idle<Player>` handshake below expresses this without dormant queued tasks.
...
fact represented is that later work must wait for the relevant task pool to drain, investigate idle
settlement instead; do not create a graph semaphore merely to schedule tasks.
...
debt remains but no tender is legal, that task stays pending and unpreparable, so the operation
