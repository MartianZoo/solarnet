...
does not choose a play policy. `FIRST` is a legitimate, deliberately unsophisticated Player policy:
it chooses the first executable task in the current presentation. A safe policy may instead stop and
ask, and a seeded random policy may choose among executable tasks without randomizing their stored
presentation. The current auto-execution implementation is engine-side, but these selection policies
belong on the client side and are intended to move there. No policy order may become an engine
guarantee or an authored precedence rule.
