...
`tfm-card-generator` turns those datasets into `cards.pets` under its build directory and appends
any authored `cards.pets` declarations for special card decks not represented by `CardDefinition`,
currently Global Events. Canon's source-generation pipeline combines that card resource with the
other authored bundle files; generated Pets are not checked in. The
