...
including a cursor inside the immediate consequences of one mutation call. A live Game World may roll
...
A Checkpoint is a coherent state at which the engine has finished one incoming mutation and is ready
to accept the next. Pending Player or Admin tasks may remain; a Checkpoint is not a policy-relative
stable point and does not mean idle.

An ordinary incoming task mutation names an Actor. That Actor's unique Agent may have received the
request from a human-facing client, script, workflow, replay, test, or its private Driver, then
invoked the engine through `ActorAccess`. The core engine neither knows nor cares which source chose
it. Once the mutation begins:

1. no other client or policy may issue another mutation;
...
5. the engine marks the resulting state as the next Checkpoint and reports mutation completion.

A generic session dispatcher observes that completion pulse and wakes the unique Agent for each
Actor. A Driver-chosen mutation is not hidden continuation of the preceding engine call: it produces
its own segment and next Checkpoint. The outermost Agent call nevertheless waits synchronously until
the dispatcher reaches a policy-relative stable point. An authored `::` effect is not autonomous
Agent behavior and remains inside the mutation that triggered it.

A Task assigned to Admin is not true-immediate merely because a normal application makes Admin fully
autonomous. It may remain pending at an intermediate Checkpoint. Admin's Agent then chooses its next
mutation, whose synchronous engine consequences lead to another Checkpoint. Player-facing Agent
calls normally return only after no Admin-assigned work remains.

Conceptually:

```text
checkpoint 3
  mutation issued by one Agent
  input event and true-immediate consequences
checkpoint 4
  generic pulse; an Agent Driver may issue another mutation
checkpoint 5
  policy-relative stable point; outer Agent call may return
```

A failed or deliberately aborted mutation leaves no events and creates no Checkpoint. A successful
mutation creates the next point even when its only retained history is diagnostic input.
...
one mutation. An arbitrary `EventCursor` is neither: it is always safe to inspect but may be unsafe
for mutation.
...
- the Checkpoint preceding their mutation segment; and
...
1. **Mutation replay.** Store the premise or source configuration plus the ordered incoming Actor
   mutations, their optional Agent/policy provenance, and required format/algorithm versions. Loading
   creates a real engine World and invokes those mutations again. This is concise and readable, but reconstruction is execution
   under the selected engine version. [ROUTINES.md](ROUTINES.md) owns the readable Routine form.
2. **State recording.** Store the mutation replay plus the complete resulting event log and
...
| --- | --- |
| Event and Checkpoint identities | Accepting and validating incoming mutations |
...
| Recording cursors and materialization caches | Disposable execution overlays |
...
Actor access depends on engine and creates passive `ActorAccess` conduits. Above access, an
application creates one sole-issuer Agent per Actor and a generic session pulse dispatcher. Neither
belongs in the state or engine ownership columns above.
...
its components, task pool, reader, and events independently, while `ActorAccess`, Agents, and their
Drivers remain explicitly tied to that World. Two speculative branches must be able to coexist.
