...
equal to the root class's own bounds:
...
Types print in minimal form. With several same-bounded dependencies, omitting an earlier unchanged
bound can make the printed form ambiguous, so those minimal forms are not round-trippable.
...
Automatic narrowing cannot discard information for a later consumer to check. Complements must leave
one candidate, and refinements on the type or any dependency bound must accept it. The result is
concrete or absent.
...
Loading a declaration as active normally activates every class name it mentions. A class name used
only as the represented class of a `Class<...>` metric is the exception: counting it does not by
itself make that class active. When the table freezes, every remaining authority-known declaration
is installed as phantom.
...
1. **Sibling nested bounds share a variable.** The same-key rule for class signatures is applied to nested
...
4. **A refined wider side always demands a world.** The refinement static relationships are not implemented
   ...
   rather than the required static rejection.
...
6. **Class signatures match only bare class names.** The intended class-signature rule adds only the
   ...
   or a `!` never becomes a variable as a unit, so matching reaches past it to the names inside. Two
   repeated `Card<Owner>` bounds should represent one choice; instead only their nested `Owner`
   occurrences are coupled, leaving the card choices free to differ. Abstractness is not consulted
   either, though a variable over concrete positions has no effect. Item 1 is the other half of
   this: one shared matching mechanism would fix both.
7. **Signature-to-effect narrowing is class-name substitution, not a variable.** The intended rule
   makes one variable span a signature and its effects; what runs is a substitution
...
10. **Conjoining refinements spreads forgiveness.** `Refinement.join` ORs the `HAS?` flag, so
    intersecting a strict and a forgiving refinement yields one forgiving conjunction whose escape
    clause can discard the strict requirement. The intended model has no greatest lower bound in
    that case.
11. **Counting a class literal activates the counted class.** The class-table model says it does not. The
    ...
    complaint, and the promised error arrives only when that metric is evaluated.
12. **A phantom dependency bound fails late.** The model treats this as a load error, but the check
    lives inside the lazily computed dependency set. Such a table builds and freezes successfully
    and throws only when something first asks the offending class for its dependencies.
13. **A nested complement does not block repetition matching.** An expression written with `!`
    should neither introduce nor refer to an implicit variable outside a class
...
14. **Matching does not canonicalize argument order.** The intended matcher compares expressions after
