...
- `TfmGameplay.playCorp` still depends on `EAGER` to settle corporation play and payment. The
  corporation-phase Pets effect bills the retained project backs in Hand when the standard
  corporation back leaves Hand; the corporation's starting benefit is available when payment
  settles. `CONCRETE` has not been verified for this path.
