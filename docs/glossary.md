...
- **BootstrapPhase:** The first Terraforming Mars `Phase`. Its presence names the initialization interval in which Modules, Players, and their minimum runtime machinery are established. `SetupPhase FROM Phase` transmutes it away to begin effectful game setup.
