...
- **Game World:** The complete live state of a game: its Component Graph, Task Queues, Event Log, Timeline, Class Table, Vocabulary, and Actor-scoped Agent, together with the Catalog and immutable premise behind them.
- **Game World Revision:**
...
- **Per:**
- **Performer:** The Actor credited on an Instruction's State Changes. Normally this is the Agent Actor, but an Instruction-level `BY` can override the Performer without changing the Task's Assignee.
