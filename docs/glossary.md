...
- **Authority:** One coherent rule universe: the Class Declarations, structured data, Vocabulary, premise rules, and exceptional Custom implementations available to a game. A game selects exactly one Authority. `Canon` is the Authority for the project's almost-published-rules version of Terraforming Mars; a rebalance would be a different Authority.
