...
Most "things" in the game are called just what you think: `Plant`, `TerraformRating`, `VictoryPoint`, and `EarthCatapult`. Pets input uses these full canonical Class Names. Tiles end in `-Tile`, like `CityTile`. Global parameters end in `-Step`, like `TemperatureStep` counts how many times the temp has been raised (from zero to 19).
